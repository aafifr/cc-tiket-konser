                ...
                @if (count($cartItems) > 0)
                        ...
                        <div class="p-6">
                            <div class="space-y-6">
                                @foreach ($cartItems as $item)
                                    <div
                                        class="flex flex-col md:flex-row md:items-start gap-4 pb-6 border-b border-gray-200">
                                            ...
                                            <div class="flex flex-col md:flex-row md:justify-between md:items-start mb-2">
                                                <div>
                                                    <h3 class="text-lg font-bold text-gray-800">
                                                        {{ $item['ticket']->event->title }}</h3>
                                                    <p class="text-gray-600 text-sm">{{ $item['ticket']->event->location }}
                                                    </p>
                                                    <div class="text-sm text-gray-500 mt-1">
                                                        {{ $item['ticket']->event->date->format('F d, Y - H:i') }}</div>
                                                    <div class="text-sm text-primary-600 font-medium mt-1">
                                                        {{ $item['ticket']->ticket_type }} Ticket</div>
                                                </div>
                                                <div class="mt-2 md:mt-0 text-right">
                                                    <span class="font-bold text-gradient text-xl">Rp
                                                        {{ number_format($item['ticket']->price, 0, ',', '.') }}</span>
                                                </div>
                                            </div>
                                            <div class="flex items-center justify-between mt-4">
                                                <div class="flex items-center">
                                                    <form action="{{ route('cart.update') }}" method="post"
                                                        ...
                                                        <label for="quantity"
                                                            class="block text-sm font-medium text-gray-700 mr-3">Qty:</label>
                                                        <select name="quantity" id="quantity"
                                                            class="w-16 rounded-md border-gray-300 focus:ring-primary-500 focus:border-primary-500 text-sm"
                                                    ...
                                                    <div class="ml-4 text-gray-700">
                                                        <span class="font-medium">Subtotal:</span> Rp
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="border-t border-gray-200 pt-3 mt-3">
                                    <div class="flex justify-between font-bold">
                                        ...
                                        <span class="text-gradient text-xl">Rp
                                            {{ number_format($total + $total * 0.11 + 50000, 0, ',', '.') }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex justify-between items-center">
                        <form action="{{ route('cart.clear') }}" method="post">
                            @csrf
                            <button type="submit"
                                class="px-6 py-2 border border-gray-300 text-gray-700 rounded-full hover:bg-gray-50 transition">
                                Clear Cart
                            </button>
                        </form>
                        <a href="{{ route('checkout') }}"
                            class="px-8 py-3 bg-gradient-primary text-white font-bold rounded-full hover:shadow-lg transition-all">
                            Proceed to Checkout
                        </a>
                    </div>
                @else
                    <div class="bg-white rounded-2xl shadow-xl p-12 text-center">
                        ...
                        <a href="{{ route('events') }}"
                            class="px-6 py-3 bg-gradient-primary text-white font-bold rounded-full hover:shadow-lg transition-all inline-flex items-center">
